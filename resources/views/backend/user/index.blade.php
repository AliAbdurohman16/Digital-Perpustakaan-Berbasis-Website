        ...
        <div class="row">
            <div class="col-12 mt-4">
                <div class="d-grid gap-2 d-md-flex">
                    <a href="{{ route('users.create') }}" class="btn btn-primary mb-3 btn-sm">Tambah Data +</a>
                </div>
                                ...
                                <tbody>
                                    ...
                                    @foreach($users as $user)
                                        <tr>
                                            ...
                                            <td class="p-3"><img src="{{ asset($user->image) }}" width="70px" class="img-fluid" alt="foto"></td>
                                            <td class="p-3">{{ $user->fullname }}</td>
                                            <td class="p-3">{{ $user->email }}</td>
                                            <td class="p-3">{{ ucfirst($user->getRoleNames()->implode(', ')) }}</td>
                                            <td style="width: 20%;">
                                                <a href="{{ route('users.edit', $user->id) }}" class="btn btn-warning btn-sm mb-2">Edit</a>
                                                <button type="button" class="btn btn-danger btn-sm mb-2 btn-delete" data-id="{{ $user->id }}">Hapus</button>
                                            </td>
                                        </tr>
                                    @endforeach
                                    <!-- End -->
